...
export class ProductService {
	...
	async getProducts(filters: any): Promise<Product[]> {
		filters = productFilterSchema.parse(filters);
		if (filters.category) {
			...
			if (productCategory) {
				...
				throw InvalidProductCategoryException(`Error listing products by category. Invalid category: ${filterData.category}`)
			}
		} else {
